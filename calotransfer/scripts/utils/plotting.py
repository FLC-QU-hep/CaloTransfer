...
X = np.load('/beegfs/desy/user/valentel/CaloTransfer/data/calo-clouds/CaloCloudsData/muon-map/X.npy')
Z = np.load('/beegfs/desy/user/valentel/CaloTransfer/data/calo-clouds/CaloCloudsData/muon-map/Z.npy')
Y = np.load('/beegfs/desy/user/valentel/CaloTransfer/data/calo-clouds/CaloCloudsData/muon-map/Y.npy')
E = np.load('/beegfs/desy/user/valentel/CaloTransfer/data/calo-clouds/CaloCloudsData/muon-map/E.npy')
...
def get_features(events):
    ...
    for layers in tqdm(events):
        ...
        for l, layer in enumerate(layers):
            ...
            layer = np.where(layer < cfg.threshold, 0.0, layer)
...
def get_observables_for_plotting(events, events_list: list = None):
...
def get_real_observables(events):
...
def get_plots_from_observables_cch(real_list, fakes_list, labels= ['1', '2', '3'], title=r'\textbf{full spectrum}', events=40_000):
    ...
    plt_radial(e_radial_real, e_radial_list, labels=labels, title=title, events=events) # radial energy profile
    plt_spinal(e_layers_real, e_layers_list, labels=labels, title=title) # mean energy per layers
    if len(real_list) != len(fakes_list):
        plt_hit_e(hits_noThreshold_list_real, hits_list, labels=labels, title=title)
    else:
        plt_hit_e(hits_real, hits_list, labels=labels, title=title) # visible cell energy
    plt_occupancy(occ_real, occ_list, labels=labels) # number of hits
    plt_esum(e_sum_real, e_sum_list, labels=labels) # energy sum
